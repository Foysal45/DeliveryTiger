...
class ServiceBillViewModel(private val repository: AppRepository): ViewModel() {

    val viewState = MutableLiveData<ViewState>(ViewState.NONE)

    fun getMerchantMonthlyReceivable(requestBody: MonthlyReceivableRequest): LiveData<MonthlyReceivableResponse> {
        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<MonthlyReceivableResponse>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.getMerchantMonthlyReceivable(requestBody)
    ...
    fun bulkMerchantCashCollection(requestBody: MonthlyReceivableUpdateRequest): LiveData<String> {
