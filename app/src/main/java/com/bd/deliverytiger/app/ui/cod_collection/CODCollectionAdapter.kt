...
class CODCollectionAdapter(var context: Context, var courierOrderViewModelList: ArrayList<CourierOrderViewModel?>?) :
    ...
    override fun onBindViewHolder(holder: myViewHolder, position: Int) {
        ...
        formattedDate = DigitConverter.toBanglaDate(courierOrderViewModelList?.get(position)?.courierOrderDateDetails?.confirmationDate.toString())

        holder.tvCodItemCount.text = DigitConverter.toBanglaDigit(position + 1)
        holder.tvCodOrderId.text = courierOrderViewModelList?.get(position)?.courierOrdersId.toString()
        holder.tvCodOrderDate.text = formattedDate

        holder.tvCodCollectionAmount.text ="৳ "+ DigitConverter.toBanglaDigit(courierOrderViewModelList?.get(position)?.courierPrice?.collectionAmount)
