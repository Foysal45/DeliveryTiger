...
class AddProductViewModel(private val repository: AppRepository) : ViewModel() {
    ...
    fun fetchAllDistricts(): LiveData<LocationResponse> {

        val responseData: MutableLiveData<LocationResponse> = MutableLiveData()

        viewState.value = ViewState.ProgressState(true)
        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.fetchAllDistricts()
            withContext(Dispatchers.Main) {
                viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
