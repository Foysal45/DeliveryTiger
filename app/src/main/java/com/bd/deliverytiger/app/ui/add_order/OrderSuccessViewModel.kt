...
class OrderSuccessViewModel(private val repository: AppRepository): ViewModel() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                when (response) {
                    is NetworkResponse.Success -> {
                        if (response.body.model != null) {
                            onOfferUpdateResponse.value = response.body.model
                        }
                    }
                    is NetworkResponse.ServerError -> {
                        val message = "দুঃখিত, এই মুহূর্তে আমাদের সার্ভার কানেকশনে সমস্যা হচ্ছে, কিছুক্ষণ পর আবার চেষ্টা করুন"
    ...
    fun sendSMS(number: String, body: String):LiveData<SMSResponse>  {
        ...
        viewState.value = ViewState.ProgressState(true)
        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.sendSMS(listOf(requestBody))
            withContext(Dispatchers.Main) {
                viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
    ...
    fun isGetOfferByMerchant(courierUserId: Int):LiveData<Boolean>  {

        viewState.value = ViewState.ProgressState(true)
        val responseBody: MutableLiveData<Boolean> = MutableLiveData()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.isGetOfferByMerchant(courierUserId)
            withContext(Dispatchers.Main) {
                viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
