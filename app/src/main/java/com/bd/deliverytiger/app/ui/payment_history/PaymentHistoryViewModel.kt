...
class PaymentHistoryViewModel(private val repository: AppRepository): ViewModel() {
    ...
    fun getPaymentHistory(courierUserId: Int): LiveData<List<PaymentData>> {
        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<List<PaymentData>>()
        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.getPaymentHistory(courierUserId)
