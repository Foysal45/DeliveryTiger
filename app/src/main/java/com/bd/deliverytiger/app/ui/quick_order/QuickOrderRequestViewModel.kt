...
class QuickOrderRequestViewModel(private val repository: AppRepository) : ViewModel() {
    ...
    fun getMerchantQuickOrders(requestBody: QuickOrderListRequest): LiveData<List<QuickOrderList>> {

        viewState.value = ViewState.ProgressState(true)
        val responseData = MutableLiveData<List<QuickOrderList>>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.getMerchantQuickOrders(requestBody)
            withContext(Dispatchers.Main) {
                viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
