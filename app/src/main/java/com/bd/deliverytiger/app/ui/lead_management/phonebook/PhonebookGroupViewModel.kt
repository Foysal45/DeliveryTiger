...
class PhonebookGroupViewModel(private val repository: AppRepository): ViewModel() {

    val viewState = MutableLiveData<ViewState>(ViewState.NONE)

    fun fetchMyPhoneBookGroup(courierUserId: Int): LiveData<List<PhonebookGroupData>> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<List<PhonebookGroupData>>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.fetchMyPhoneBookGroup(courierUserId)
