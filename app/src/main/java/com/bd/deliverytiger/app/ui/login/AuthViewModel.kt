...
class AuthViewModel(private val repository: AppRepository) : ViewModel() {
    ...
    fun fetchCategory(): LiveData<List<CategoryData>> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<List<CategoryData>>()
    ...
    fun fetchSubCategoryById(categoryId: Int): LiveData<List<SubCategoryData>> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<List<SubCategoryData>>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.fetchSubCategoryById(categoryId)
            withContext(Dispatchers.Main) {
                viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
